...
Initially, we identified in our problem formulation (section \ref{section:problem formulation}) that there is a gap in programming language evaluation between internal language analysis and the big industrial evaluation methods. In that regard, as well as a follow-up of our previous work (\chapref{chapter:previous work}), we wanted to explore whether HCI techniques could be applicable for the evaluation of programming languages. 

In \chapref{chap:eval_methods}, we briefly described existing evaluation methods for programming languages and how they differ, along with their strengths and weaknesses, which helped us get a better understanding of the design of such methods.
In the context of HCI techniques, we have examined the usefulness of the discount usability method ( \chapref{chapter:ExperimentEvaluation}) the IDA method (\secref{section:IDA}) for data analysis on C\# and F\#.
...
To create a method better suited for evaluating language design, we conducted an adapted usability experiment where we specifically avoided the use of an IDE or a compiler (\chapref{chapter:MethodIntro}).
...
\input{./text/method/results/MethodProcedure.tex}
